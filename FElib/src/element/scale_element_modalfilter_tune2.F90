...
module scale_element_modalfilter
  ...
  type, public :: ModalFilter
    ...
    real(RP), allocatable :: FilterMat_tr(:,:)
    real(RP), allocatable :: FilterMat_v_tr(:,:)
  contains
...
contains
  ...
  subroutine ModalFilter_Init_hexahedral( this,  & ! (inout)
    ...
    type(LineElement) :: elem1D, elem1D_v
    !--- end
    integer :: p1, p2
    integer :: l
    logical :: tend_flag_
    ...
    call elem1D_v%Init(elem%PolyOrder_v, .false.)

    allocate( this%FilterMat(elem1D%Np,elem1D%Np) )
    allocate( this%FilterMat_tr(elem1D%Np,elem1D%Np) )
    allocate( this%FilterMat_v_tr(elem1D_v%Np,elem1D_v%Np) )
    this%FilterMat(:,:) = 0.0_RP
    this%FilterMat_tr(:,:) = 0.0_RP
    this%FilterMat_v_tr(:,:) = 0.0_RP
    ...
    do p2=1, elem1D_v%Np
      this%FilterMat_v_tr(p2,p2) = filter1D_h(p2)
    end do

    !-- get filter matrix in x direction
    this%FilterMat(:,:) = matmul(this%FilterMat, elem1D%invV)
    this%FilterMat(:,:) = matmul(elem1D%V, this%FilterMat)
    !-- get filter matrix in y direction
    this%FilterMat_tr(:,:) = transpose(this%FilterMat(:,:)) 
    !-- get filter matrix in z direction
    this%FilterMat_v_tr(:,:) = matmul(this%FilterMat_v_tr, elem1D_v%invV)
    this%FilterMat_v_tr(:,:) = matmul(elem1D_v%V, this%FilterMat_v_tr)
    this%FilterMat_v_tr(:,:) = transpose(this%FilterMat_v_tr(:,:))
